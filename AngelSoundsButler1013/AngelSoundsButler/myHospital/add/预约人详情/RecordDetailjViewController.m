...
#import "RecordDetailjViewController.h"

@interface RecordDetailjViewController ()

@end

@implementation RecordDetailjViewController
...
-(void)makeUI
{
    table = [[UITableView alloc]initWithFrame:CGRectMake(0, 74, kSCREEN_WIDTH, self.view.frame.size.height-74)style:UITableViewStylePlain];
    table.delegate = self;
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        if ([dic objectForKey:@"resultImgUrl"]  != nil) {
            ...
            [cell.tupian sd_setImageWithURL:[NSURL URLWithString:[dic objectForKey:@"resultImgUrl"]] placeholderImage:kImage(@"chat_plus_photo_h") completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
            }];
...
-(void)btnWithImage:(UIButton*)btn
{
    ...
    bttn.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height-64);
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        NSDictionary *dic = _dataArr[indexPath.row];
        a = 173+[[dic objectForKey:@"resultContent"] sizeWithFont:[UIFont systemFontOfSize:14.0] maxSize:CGSizeMake(280, MAXFLOAT)].height+10;
    }
    return  a;
}
