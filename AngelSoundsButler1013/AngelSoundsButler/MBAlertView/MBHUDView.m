...
+(MBHUDView*)hudWithBody:(NSString*)body type:(MBAlertViewHUDType)type hidesAfter:(float)delay show:(BOOL)show
{
    ...
    alert.backgroundColor = RGBA(0, 0, 0, 0.7);
    ...
    if(type == MBAlertViewHUDTypeExclamationMark)
    {
        ...
        alert.iconLabel.font = [UIFont boldSystemFontOfSize:14.0];
...
-(UIFont*)bodyFont
{
    if(_bodyFont)
        return _bodyFont;
    CGFloat size = 0;
    [self.bodyText sizeWithFont:[UIFont systemFontOfSize:14.0] minFontSize:12.0 actualFontSize:&size forWidth:self.contentRect.size.width / 1.3 lineBreakMode:NSLineBreakByTruncatingTail];
...
-(UIButton*)bodyLabelButton
{
    ...
    CGSize size = CGSizeMake(180,54);
    labl.numberOfLines = 0;
    labl.text = self.bodyText;
    labl.font = font;
    
    NSDictionary * tdic = [NSDictionary dictionaryWithObjectsAndKeys:font,NSFontAttributeName,nil];
    CGSize  lablesize =[self.bodyText boundingRectWithSize:size options:NSStringDrawingUsesLineFragmentOrigin  attributes:tdic context:nil].size;
    [labl setFrame:CGRectMake(8, bounds.size.height/2.0 - size.height/2.0 - 8, 180-16, lablesize.height)];
...
//    _bodyLabelButton = [[UIButton alloc] initWithFrame:CGRectMake(bounds.origin.x + bounds.size.width/2.0 - size.width/2.0, bounds.size.height/2.0 - size.height/2.0 - 8, size.width, size.height)];
    _bodyLabelButton = [[UIButton alloc] initWithFrame:CGRectMake(bounds.origin.x + bounds.size.width/2.0 - 90, bounds.size.height/2.0 - 90 - 8, lablesize.width, lablesize.height)];
...
//    _bodyLabelButton.titleLabel.numberOfLines = 2;
//    
//    _bodyLabelButton.titleLabel.textAlignment = NSTextAlignmentCenter;
    [_bodyLabelButton addSubview:labl];
...
-(void)layoutView
{
    ...
    {
        _activityIndicator = [MBSpinningCircle circleWithSize:NSSpinningCircleSizeLarge color:[UIColor colorWithRed:255.0/255.0 green:126.0/255.0 blue:165.0/255.0 alpha:1.0]];
    ...
    {
        _checkMark = [MBCheckMarkView checkMarkWithSize:MBCheckmarkSizeMedium color:[UIColor whiteColor]];
...
-(void)centerViews
{
    ...
    {
        [_backgroundButton centerViewsVerticallyWithin:@[@{@"view" : _checkMark, @"offset" : [NSNumber numberWithFloat:self.iconOffset.height]}, @{@"view" : self.bodyLabelButton, @"offset" : [NSNumber numberWithFloat:5]}]];
    }
    else if(_hudType == MBAlertViewHUDTypeLabelIcon || self.iconLabel.text)
    {
